...
struct hid_device_info * hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
 ...
 for (;;) {
  ...
  if ((vendor_id == 0x0 && product_id == 0x0) || 
   ...
   if (res)  cur_dev->serial_number = wcsdup(wstr);
...
void chubRUN() {
 ...
 //printf("conk\n");
 while(handhold != NULL) {
 res = ReadFile(handhold->device_handle, gwonz, handhold->input_report_length, &bytes_read, NULL);//&handhold->ol);
 //printf("conk\n");
 if (res)
...
void chubSENDz(unsigned char * rept, int amountus) {
 static unsigned char quonz[9]; DWORD bytes_written;
 //quonz = (unsigned char *)malloc(amountus+1);
 if (amountus == 1) quonz[0] = 0;
 else quonz[0] = 0;
 ...
 while ((handhold != NULL) && (handhold->device_handle)) {
  ...
  //res = WriteFile(handhold->device_handle, quonz, amountus+1, &bytes_written,NULL);//&bytes_written, NULL); //NULL,&ol
  //res = WriteFile(handhold->device_handle, quonz, amountus+1, NULL,&ol);//&bytes_written, NULL); //NULL,&ol
  //res = GetOverlappedResult(handhold->device_handle, &ol, &bytes_written, TRUE/*wait*/);
  if (amountus == 8) {
   res = HidD_SetFeature(handhold->device_handle, (PVOID)quonz, amountus+1);
   //printf("  innnloop\n");
   if (res)  return; 
  } /*else {
   res = HidD_GetFeature(handhold->device_handle, (PVOID)quonz, amountus+1);
   //printf("  innnloop\n");
   if (res)  return; 
  }*/
  
  ///////if (amountus < 8) return;
  //return 1;
 }
}
